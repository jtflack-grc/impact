...
const FAIR_METRICS: Set<ScenarioMetricKey> = new Set([
  "controlAdoption",
  "incidentRate",
]);

/** One-line FAIR takeaway from metric deltas for the impact popup */
function fairTakeaway(
  deltas: Partial<Record<ScenarioMetricKey, number>>
): string | null {
  ...
  if (deltas.incidentRate != null && deltas.incidentRate < 0) {
    lines.push(
      "Threat event frequency (TEF) reduced → lower loss event frequency (LEF)."
    );
  }
...
const REVEAL_INTERVAL_MS = 4; // Fast progressive briefing reveal
const CHARS_PER_REVEAL = 2;
const PAUSE_BETWEEN_BRIEFING_LINES_MS = 40;
...
export function ChatPanel() {
  ...
  const [briefingEntries, setBriefingEntries] = useState<BriefingEntry[]>([]);
  ...
  useEffect(() => {
    ...
    const initialEntries: BriefingEntry[] = messages.map((msg, idx) => ({
      id: `msg-${idx}`,
      text: msg,
      displayedLength: 0,
      isComplete: false,
    }));
    setBriefingEntries(initialEntries);
    setIsRevealing(true);
    ...
    const intervalId = setInterval(() => {
      ...
      if (charIdx >= currentText.length) {
        ...
        charIdxRef.current = 0;
        setCurrentBriefingIndex(msgIdx + 1);
        if (msgIdx + 1 >= msgs.length) {
          clearInterval(intervalId);
          setIsRevealing(false);
          return;
        }
        pauseUntil = Date.now() + PAUSE_BETWEEN_BRIEFING_LINES_MS;
        return;
      }

      const advance = Math.min(CHARS_PER_REVEAL, currentText.length - charIdx);
      charIdxRef.current = charIdx + advance;
      setBriefingEntries((prev) => {
        const updated = [...prev];
        ...
        {simulationComplete ? (
          ...
          <div className="space-y-6">
            <div className="rounded-xl bg-gradient-to-br from-emerald-900/40 to-slate-900/80 border border-emerald-500/40 px-6 py-6 text-center">
              ...
              <p className="text-sm text-slate-300 mb-6">
                You completed all 15 scenarios. Thank you for exploring FAIR and
                FMVA with IMPACT!
              </p>
              <div className="text-left max-w-md mx-auto space-y-3 mb-6">
                ...
                <ul className="space-y-2 text-sm text-slate-300 list-disc list-inside">
                  <li>
                    FAIR loss percentiles (P90, LEF, loss magnitude) across 15
                    scenarios
                  </li>
                  <li>
                    FMVA-style capital budgeting (NPV, IRR, ROSI) and
                    revenue-at-risk thinking
                  </li>
                  <li>
                    How choices affect control adoption and threat event
                    frequency (TEF) in the dashboard
                  </li>
                </ul>
              </div>
          ...
          <>
            ...
            <div className="rounded-xl bg-black/60 border border-war-border/70 px-4 py-3 space-y-2">
              ...
              <h2 className="text-sm font-semibold text-war-white">
                {scenario.title}
              </h2>
              ...
              <p className="text-xs text-war-muted/90">
                ...
                <span className="font-semibold text-war-white/90">
                  {scenario.company.name}
                </span>
                , a {scenario.company.sector.toLowerCase()} operating in{" "}
                {scenario.company.region}. Annual revenue is approximately{" "}
                <span className="font-semibold">
                  ${scenario.company.annualRevenueMillions.toLocaleString()}M
                </span>{" "}
                with an EBITDA margin around{" "}
            ...
            {tailSelection?.scenarioId === scenario.id && (
              <div className="border border-red-400/35 bg-red-950/10 px-4 py-3">
                ...
                <div className="mt-1 flex items-baseline justify-between gap-3">
                  <div className="text-xs font-semibold text-war-white">
                    {tailSelection.label}
                  </div>
                ...
                <p className="mt-1 text-[11px] leading-relaxed text-war-muted">
                  Selected losses span ${tailSelection.lowMillions.toFixed(0)}
                  M–${tailSelection.highMillions.toFixed(0)}M with a conditional
                  mean of ${tailSelection.conditionalMeanMillions.toFixed(1)}M.
                  That is about {tailSelection.ebitdaSharePercent.toFixed(0)}%
                  of annual EBITDA.
                </p>
              </div>
            )}

            {/* Briefing feed — reveal entries in authored order */}
            <div className="space-y-3">
              {briefingEntries.map((entry, idx) => {
                if (idx > currentBriefingIndex) return null;
                const displayedText = entry.text.slice(
                  0,
                  entry.displayedLength
                );
                const isCurrentlyRevealing =
                  idx === currentBriefingIndex &&
                  isRevealing &&
                  !entry.isComplete;
                return (
                  <div
                ...
                <div className="space-y-3">
                  <div className="rounded-xl bg-slate-800/90 border border-slate-600 shadow-xl overflow-hidden">
                    ...
                    <div className="px-4 py-3">
                      ...
                      Object.keys(lastChoiceImpact.metricDeltas).length > 0 ? (
                        <>
                          {/* Show only FAIR metrics prominently */}
                          {Object.entries(lastChoiceImpact.metricDeltas).some(
                            ([key]) =>
                              FAIR_METRICS.has(key as ScenarioMetricKey)
                          ) && (
                            <ul className="space-y-2 mb-4">
                              {(
                                Object.entries(
                                  lastChoiceImpact.metricDeltas
                                ) as [ScenarioMetricKey, number][]
                              )
                          ...
                          {Object.entries(lastChoiceImpact.metricDeltas).some(
                            ...
                            <p className="text-[10px] text-slate-500 mb-4 italic">
                              Other scenario factors may also change (see center
                              dashboard for full FAIR/FMVA analysis).
                            </p>
                          )}
                        </>
                      ) : (
                        <p className="text-sm text-slate-400 mb-4">
                          Check the center dashboard for updated FAIR risk
                          metrics and FMVA financial impact.
                        </p>
                      )}
                      {lastChoiceImpact?.metricDeltas &&
                        fairTakeaway(lastChoiceImpact.metricDeltas) && (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="font-semibold text-war-white group-hover:text-white mb-1">
                                {choice.label}
                              </div>
                              {choice.summary && (
                              ...
                              {!choice.summary && choice.metricDeltas && (
                                <div className="text-[11px] text-slate-500 mt-1">
                                  This choice will update FAIR metrics (e.g.
                                  control adoption, LEF) and FMVA impact in the
                                  center dashboard.
                                </div>
                              )}
                            </div>
