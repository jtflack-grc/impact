...
const FmvaSpreadsheet = lazy(() =>
  import("./FmvaSpreadsheet").then((module) => ({
    default: module.FmvaSpreadsheet,
  }))
);
...
export function MetricsDashboard() {
  ...
  const dynamicLossProfile = useMemo(() => {
    ...
    const vulnerabilityFactor =
      (100 - activeMetrics.controlAdoption) / (100 - baseControlAdoption);
    const adjustedFrequency =
      base.frequencyPerYear * Math.max(0.3, Math.min(1.7, vulnerabilityFactor));
  ...
  const revenueAtRiskData = useMemo(() => {
    ...
    return [
      ...
      {
        metric: "Gross P90",
        value: dynamicLossProfile.grossP90,
        fill: "#ef4444",
      },
  ...
  const companyEbitda =
    companyRevenue * (scenario.company.ebitdaMarginPercent / 100);
  const p90AsPctRevenue = useMemo(
    () =>
      companyRevenue > 0
        ? (dynamicLossProfile.grossP90 / companyRevenue) * 100
        : 0,
  ...
  const p90AsPctEbitda = useMemo(
    () =>
      companyEbitda > 0
        ? (dynamicLossProfile.grossP90 / companyEbitda) * 100
        : 0,
  ...
  const capitalBudgeting = useMemo(() => {
    const annualBenefit =
      dynamicLossProfile.grossP90 - dynamicLossProfile.netP90;
    const controlCost3yr = Math.max(
      2,
      scenario.company.annualRevenueMillions * 0.002 * 3
    ); // ~0.2% of revenue per year × 3
    return {
      controlCost3yr,
      annualBenefit,
      ...
      <div className="px-6 pt-5 pb-3 border-b border-war-border/80 flex items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] tracking-[0.2em] text-war-muted">
              FAIR & FMVA Analysis
            </span>
            <EduTooltip
      ...
      >
              ...
              <section className="space-y-2">
                ...
                <div className="mt-4 rounded-xl border border-war-border/80 bg-black/60 p-4">
                  ...
                  <p className="text-[10px] text-war-muted mt-2">
                    Key FAIR-style percentiles: P50 (typical outcome) and P90
                    (conservative tail loss where 90% of outcomes fall below
                    this amount).
                  </p>
                </div>
                <div className="mt-4 rounded-xl border border-war-border/80 bg-black/60 p-4">
                  ...
                  <p className="text-[10px] text-war-muted mt-2">
                    Gross P90 as % of EBITDA:{" "}
                    {scenario.company.annualRevenueMillions *
                      (scenario.company.ebitdaMarginPercent / 100) >
                    0
                      ? (
                          (dynamicLossProfile.grossP90 /
                            (scenario.company.annualRevenueMillions *
                              (scenario.company.ebitdaMarginPercent / 100))) *
                          100
                        ).toFixed(0)
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-[10px]">
                <div className="rounded-lg bg-black/60 p-3 border border-war-border/40">
                  ...
                  <p className="text-[10px] text-war-muted mt-1">
                    90th percentile loss before insurance and recovery—tail risk
                    for planning.
                  </p>
                </div>
                <div className="rounded-lg bg-black/60 p-3 border border-war-border/40">
                  ...
                  <p className="text-[10px] text-war-muted mt-1">
                    Residual 90th percentile loss after modeled insurance and
                    buffers.
                  </p>
                </div>
                <div className="rounded-lg bg-black/60 p-3 border border-war-border/40">
                  <div className="text-war-muted mb-1">
            ...
            <div className="mt-4 rounded-xl border border-war-border/80 bg-black/60 p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-[10px]">
                <div className="flex gap-2">
                  <span className="text-red-400/90 font-medium shrink-0">
                    EAL
                  </span>
                  <span className="text-war-muted">
                    Expected Annual Loss = mean loss × LEF; average $ loss per
                    year for reserves and planning.
                  </span>
                </div>
                <div className="flex gap-2">
                  <span className="text-sky-400/90 font-medium shrink-0">
                    P90
                  </span>
                  <span className="text-war-muted">
                    90th percentile loss; 90% of outcomes fall below this
                    amount. Used as tail risk for planning.
                  </span>
                </div>
                <div className="flex gap-2">
                  ...
                  <span className="text-war-muted">
                    Loss Event Frequency = TEF × Vulnerability; events per year
                    in this scenario.
                  </span>
                </div>
                <div className="flex gap-2">
                  <span className="text-war-white/80 font-medium shrink-0">
                    Gross / Net P90
                  </span>
                  <span className="text-war-muted">
                    Gross = before insurance/controls; Net = after. The gap is
                    risk reduction for ROSI and capital budgeting.
                  </span>
                </div>
                <div className="flex gap-2 sm:col-span-2">
                  <span className="text-war-muted italic">Bridge:</span>
                  <span className="text-war-muted">
                    FAIR gives P90 and EAL; FMVA puts them in business terms
                    (P90 as % revenue, P90 as % EBITDA) so boards and finance
                    see risk in the same language as the P&L.
                  </span>
                </div>
              </div>
            </div>
        ...
        {activeTab === "enforcement" && (
          <section
            className="space-y-2"
            role="tabpanel"
            id="enforcement-tabpanel"
            aria-labelledby="enforcement-tab"
          >
            <h2 className="flex items-center gap-1.5 text-xs font-semibold tracking-[0.18em] text-war-muted">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="space-y-1">
                  ...
                  <div className="text-[10px] text-war-muted/80">
                    In FAIR, LEF = TEF × Vulnerability. Here LEF is the modeled
                    events/year and Vulnerability is a rough proxy based on
                    control adoption (~
                    {((1 - activeMetrics.controlAdoption / 100) * 100).toFixed(
                      0
                    )}
                    %).
                  </div>
                  <div className="text-[9px] text-war-muted/60 italic mt-1">
                    TEF is reflected in the scenario base and in choice impacts
                    (incident rate); Vulnerability is approximated by control
                    adoption.
                  </div>
                </div>
                <div className="space-y-1">
                  ...
                  <div className="text-[10px] text-war-muted/80">
                    Primary:{" "}
                    {formatMillions(dynamicLossProfile.meanLoss * 0.65)}
                    <br />
                    Secondary:{" "}
                    {formatMillions(dynamicLossProfile.meanLoss * 0.35)}
                    <br />
                    <span className="text-[9px] text-war-muted/60 italic">
                      (Illustrative breakdown; actual ratios vary by scenario)
                    </span>
                  </div>
                </div>
              </div>
              <div className="mt-3 pt-3 border-t border-war-border/50">
                <div className="text-[10px] text-war-muted mb-2">
            ...
            <div className="rounded-xl border border-war-border/80 bg-black/60 p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-5 gap-2 text-[10px] border-t border-war-border/30 pt-1">
                  ...
                  <div className="text-war-white text-center">
                    ${scenario.company.annualRevenueMillions.toFixed(0)}M
                  </div>
                  <div className="text-war-white text-center">
                    $
                    {(scenario.company.annualRevenueMillions * 1.05).toFixed(0)}
                    M
                  </div>
                  <div className="text-war-white text-center">
                    ${(scenario.company.annualRevenueMillions * 1.1).toFixed(0)}
                    M
                  </div>
                  <div className="text-war-white text-center">
                    $
                    {(scenario.company.annualRevenueMillions * 1.15).toFixed(0)}
                    M
                  </div>
                </div>
                <div className="grid grid-cols-5 gap-2 text-[10px]">
              ...
              <p className="text-[10px] text-war-muted mt-2">
                Stress: if a P90 loss occurs in a given year, it would reduce
                EBITDA by that amount (one-time loss).
              </p>
            </div>
                  ...
                  <div className="text-sm font-semibold text-war-white">
                    ...
                    {deepFinanceWacc !== null && (
                      <span className="text-[10px] text-emerald-400 ml-2">
                        (from Deep Finance Mode)
                      </span>
                    )}
                  </div>
                </div>
                <div>
                  <div className="text-[10px] text-war-muted mb-1">
                    Terminal growth
                  </div>
                  <div className="text-sm font-semibold text-war-white">
                    2.5%
              ...
              <ResponsiveContainer width="100%" height={120}>
                <BarChart
                  data={[
                    ...
                    {
                      scenario: "Upside",
                      value: dcfValuation.equityValue * 1.2,
                    },
                    {
                      scenario: "Downside",
                      value: dcfValuation.equityValue * 0.8,
                    },
                  ]}
                >
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
              ...
              <div className="space-y-2">
                <div className="grid grid-cols-2 gap-3 text-[10px]">
                  ...
                  <div>
                    ...
                    <div className="text-sm font-semibold text-war-white">
                      {formatMillions(capitalBudgeting.controlCost3yr)}
                    </div>
                  </div>
                </div>
                <div className="pt-2 border-t border-war-border/30">
                  ...
                  <div className="text-lg font-bold text-emerald-400">
                    {(capitalBudgeting.controlCost3yr > 0
                      ? (((dynamicLossProfile.grossP90 -
                          dynamicLossProfile.netP90) *
                          3 -
                          capitalBudgeting.controlCost3yr) /
                          capitalBudgeting.controlCost3yr) *
                        100
                      : 0
                    ).toFixed(0)}
                  ...
                  <div className="text-[10px] text-war-muted/80 mt-1">
                    ...
                    <br />
                    <span className="text-[9px] text-war-muted/60 italic">
                      (Simplified ROSI: assumes deterministic events.) For
                      rigorous comparison, use NPV with your WACC—it accounts
                      for discounting and probability.
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="rounded-xl border border-war-border/80 bg-black/60 p-4">
              ...
              <p className="text-[10px] text-war-muted mb-3">
                Benefit = annual risk reduction (Gross P90 − Net P90); cost =
                control investment over 3 years.
              </p>
              ...
              <div className="text-[10px] text-war-muted">
                NPV = PV(3yr risk reduction) − control cost; discount rate{" "}
                {(effectiveWacc * 100).toFixed(1)}% (WACC). Control cost ≈ 0.2%
                of revenue × 3yr.
              </div>
            </div>
            ...
            <div className="rounded-xl border border-war-border/80 bg-black/60 p-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-war-white font-semibold">
                    {activeMetrics.controlAdoption}%
                  </span>
                </div>
        ...
        {activeTab === "fmva" && (
          <section
            className="flex-1 flex flex-col min-h-0 min-w-0 space-y-4"
            role="tabpanel"
            id="fmva-tabpanel"
            aria-labelledby="fmva-tab"
          >
                  ...
                  fallback={
                    <div className="flex-1 flex items-center justify-center min-h-[400px]">
                      <div className="text-center space-y-3">
                        ...
                        <p className="text-sm text-war-muted">
                          Loading FMVA Model...
                        </p>
                      </div>
                    </div>
...
function MetricCard({
  ...
  const clamped =
    fraction === undefined ? 0.75 : Math.max(0, Math.min(1, fraction));
